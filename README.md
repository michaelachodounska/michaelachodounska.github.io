...
The old site hosted images on Google's servers, so I couldn't copy the
files themselves — only reference them. Save the two homepage photos
and the portfolio photo from your current site (right-click → save
image as, or screenshot at full resolution) and drop them into
`assets/` with these exact names:

- `assets/homepage-1.jpg` — the top image on the homepage
- `assets/homepage-2.jpg` — the second homepage image
- `assets/portfolio-1.jpg` — the portfolio page image

If you'd rather use different filenames, just update the `src=` in
the matching HTML file to match.
...
- The CV and portfolio PDF links still point to Google Drive, same
  as the original site. If you'd rather host those files in the
  repo too, drop them in (e.g. `assets/cv.pdf`) and update the
  `href` in `index.html` / `portfolio.html` to the local path.
